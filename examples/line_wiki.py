...
def main(smoke=False, show=True):
    ...
    model = LINE(graph, embedding_size=8 if smoke else 128, order="second")
    model.train(batch_size=2 if smoke else 1024, epochs=1 if smoke else 50, verbose=0 if smoke else 2)
